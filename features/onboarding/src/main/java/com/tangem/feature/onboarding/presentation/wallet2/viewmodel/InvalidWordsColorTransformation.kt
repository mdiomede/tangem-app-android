...
class InvalidWordsColorTransformation(
    ...
    override fun filter(text: AnnotatedString): TransformedText {
        ...
        log("")
        log("")
        val logWordsToBrush = wordsToBrush.joinToString { "[$it]" }
        log("wordsToBrush: $logWordsToBrush")

        val wordList = makeSeparatedWordlist(plainText)
        val logWordList = wordList.joinToString { "[$it]" }
        log("wordlist: $logWordList")
        val annotatedString = makeAnnotatedString(wordList)
        log("annotatedString: ${annotatedString.text}")
