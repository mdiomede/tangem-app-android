...
/**
 * @author Andrew Khokhlov on 04/11/2023
 */
...
internal class WalletViewModelV2 @Inject constructor(
    ...
    private fun initializeWallets(action: WalletsUpdateActionResolverV2.Action.InitializeWallets) {
        ...
        stateHolder.update(
            transformer = InitializeWalletsTransformer(action = action, clickIntents = clickIntents),
        )
    ...
    private fun unlockWallet(action: WalletsUpdateActionResolverV2.Action.UnlockWallet) {
        viewModelScope.launch(dispatchers.main) {
            ...
            stateHolder.update(
                transformer = UnlockWalletTransformer(action = action, clickIntents = clickIntents),
            )
